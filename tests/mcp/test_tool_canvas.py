...
async def test_create_canvas(app, project_config):
    ...
    # Execute
    result = await (canvas.fn if hasattr(canvas, "fn") else canvas)(nodes=nodes, edges=edges, title=title, folder=folder)
...
async def test_update_existing_canvas(app, project_config):
    ...
    await (canvas.fn if hasattr(canvas, "fn") else canvas)(nodes=nodes, edges=edges, title=title, folder=folder)
    ...
    # Execute update
    result = await (canvas.fn if hasattr(canvas, "fn") else canvas)(nodes=updated_nodes, edges=updated_edges, title=title, folder=folder)
